// Replay mode for the HTTP client.
//
// When enabled, fetchScoreboard / fetchSummary in sports-api.js read JSON
...
function resetState(newDir) {
    ...
    _counters = new Map();
    if (newDir)
        console.debug(`[GnomeFootball] replay mode ON: ${newDir}`);
    else
        console.debug('[GnomeFootball] replay mode OFF');
}
...
export async function replayScoreboard(slug) {
    const root = expandPath(_replayDir);
...
export async function replaySummary(slug, eventId) {
