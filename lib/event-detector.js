...
// ESPN soccer /summary returns significant moments in `keyEvents`, not `plays`
...
export function detectEvents({ scoreboardEvent, summary, previousState, enabledEvents }) {
    ...
    if (previousState == null && (state === MATCH_STATE.IN || state === MATCH_STATE.POST)) {
        ...
        console.log(`[GnomeFootball] cold-start baseline ${eventId} (state=${state}, period=${period}, plays=${baselinePlayIds.length}) — suppressing catch-up notifications`);
